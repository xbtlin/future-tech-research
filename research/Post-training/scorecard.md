...
### 清华质疑（arXiv 2504.13837）本季被两篇独立论文实证坐实——但两篇都给了修法

| 论文 | 日期 | 核心结论 |
|---|---|---|
| [arXiv 2607.20543](https://arxiv.org/abs/2607.20543)《When RLVR Shrinks the Reasoning Boundary: Diagnosing Pass@k Inversion》 | 2026-07-12 | 直接命名"**pass@k 反转**"：RLVR 提升单次准确率但**恶化重复采样表现**。机制：问题出在"边界提示"——基座里那些**低频但正确**的解在被充分强化前就先消失了。提出 Per-Problem Base Anchoring (PBA)，三个种子在 Omni-MATH-Test 上 pass@1 与高预算覆盖率相对匹配 GRPO 双双改善 |
| [arXiv 2606.15455](https://arxiv.org/abs/2606.15455)《Understanding Diversity Collapse in RLVR via the Lens of Overtraining》 | 2026-06-13 | 把"pass@1 升、高 k 的 pass@k 降"重新框定为**过训练**而非能力上限。**关键反证：有相当一部分初始不可解的问题在标准 RLVR 训练中变成了可解。** 提出 Bayesian Boundary Gating (BBG)，并给出可证伪强结论：**把更新限制在"零成功"问题上，Pass@256 能在困难 benchmark 上超过基座模型** |

**怎么读**：清华质疑的**现象描述**（pass@k 会退化）被独立坐实，这对 Q2 记的反面观点是加分；但清华质疑的**推论**（"RLVR 只是榨油水、能力边界推不动"）被 2606.15455 的"零成功门控可把 Pass@256 推过基座"直接挑战。

**三条限定**：两篇均为**单实验室、预印本、无独立复现**；两篇的方法本质是同一思路（别在会伤害多样性的地方更新）的两个变体，**不算互相独立的验证**；两篇都**没有在 ≥30B 的工业级规模上复现**。

**注意——一条被否决的说法**：arXiv 2606.22317（boundary-aware Curriculum RL，声称 pass@256 相对基座 +9.8pt）曾被当作对清华质疑的反驳，但在 3 票对抗核查中被 **0-3 否决**，本仓库不得引用。

### 判断修正

- **星级不改**：现象与修法都还在预印本阶段，未到工业规模
- **命题 1（self-play 成为数据引擎）：连续两轮无信号。** 若 Q4 仍无 ≥30B 复现，应下调该命题概率
- **命题 2（Generative/Thinking PRM 融合）：方向存疑。** 本季两篇论文指向的是"边界门控/防过训练"，不是 PRM 路线
- **信号 1（GRPO/DAPO vs PPO head-to-head）本季无新公开对比**
