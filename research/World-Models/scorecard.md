...
[WorldRoamBench](https://arxiv.org/abs/2606.31672)（arXiv 2606.31672，2026-06-30 提交 / 07-06 修订）：**面向交互式世界模型长时程稳定性**的开放世界基准。四个维度——逐帧动作指标、分段视觉漂移检测、以可控性为门控的物理评估（力学/光学/3D 一致性）、以点云重建与 VLM 推理做记忆评估。600+ 测试用例，自然/城市/室内三类场景，**10—60 秒连续交互**，第一与第三人称。

**结论：评测的 10 多个模型中，没有一个能在所有维度上可靠达标，最好的也只是中等分数。**

**对命题 2（一致性窗口 1 分钟 → 30 分钟+）的影响：转负向。** 新量具的测试区间只到 60 秒，而**在这个区间内就已经没有模型全维度达标** ——距离 30 分钟差两个数量级以上。原跟踪信号"Cosmos / Genie 一致性时长（1min → 10min 是关键门槛）"应改用 WorldRoamBench 的分维度分数，而非厂商自报时长。

### VLA：首次有了成套的可比数字

[Qwen-VLA](https://arxiv.org/abs/2605.30280)（arXiv 2605.30280，2026-05-28 / 06-01 修订）：统一具身基座模型，DiT 动作解码器，跨任务/环境/机器人本体。
...
**要看的是最后两行**：同一个模型，**仿真 97.9% → 真机分布外 76.9%（掉 21 分）→ 零样本动态操作 26.6%（掉 71 分）** 。Q2 记的"sim-to-real 与长尾泛化是核心瓶颈"，本季首次有了精确的缺口数字。

### 人形机器人产能：本季最扎实的制造业数字

[Figure 官方](https://www.figure.ai/news/ramping-figure-03-production)（2026-04-29，略晚于 Q2 基线）：

- 生产节拍**从 1 台/天提到 1 台/小时** ——120 天内 **24 倍**吞吐提升
- 已交付**超过 350 台**第三代人形机器人
- 下线首次通过率 **>80%** ；电池线首次通过率 99.3%、出货 500+ 电池包
- 已生产 **9,000+ 执行器**、10 个以上 SKU；每台出厂前经 80+ 项功能验证
- 150+ 联网工位、50+ 在线检测点

**限定：这是产能与良率，不是需求，也不是任务成功率。** 官方未披露订单、客户数或收入。**命题 1（2029 年工厂成功率 95%+、部署 100 家工厂）：产能侧正向，能力侧无数据。**

**1X NEO 与 Tesla Optimus 的窗口内交付数据本轮未取证** ——是"未查到"，不是"没进展"。
